...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.4.2', __( 'WooCommerce Customer/Order CSV Export', 'woocommerce-customer-order-csv-export' ), __FILE__, 'init_woocommerce_customer_order_csv_export', array(
...
class WC_Customer_Order_CSV_Export extends SV_WC_Plugin {
	...
	protected function install() {
		...
		// install default custom column mapping settings
		require_once( $this->get_plugin_path() . '/includes/admin/class-wc-customer-order-csv-export-admin-column-mapper.php' );

		foreach ( WC_Customer_Order_CSV_Export_Admin_Column_Mapper::get_settings() as $section => $settings ) {
